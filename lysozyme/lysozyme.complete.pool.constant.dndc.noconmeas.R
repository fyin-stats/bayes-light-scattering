...
########################################
# they share the same design concentration matrix
########################################
...
#
write.model(lysozyme_model, "./lysozyme/lysozyme_model_log_normal.bug")
############################
time1 <- Sys.time()
...
out <- do.call(jags.parallel, 
               list(data=dat_jags, inits=jags.inits, 
                    ...
                    n.thin = thin, 
                    digits = 16, 
                    model.file="./lysozyme/lysozyme_model_log_normal.bug"))
...
time2 <- Sys.time()
####
saveRDS(out, "./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior.rds")
...
out <- readRDS("./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior.rds")
out_mcmc <- as.mcmc(out)
...
# ########################
# ########################
pdf("./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_shorter_residuals_LS.pdf")
...
dev.off()
# ##########################
pdf("./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_shorter_residuals_dRI.pdf")
...
dev.off()
#########################
pdf("./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_shorter_residuals_con.pdf")
...
########################
#######################
# posterior predictive value for each data entry
dRI_df_post <- dRI_df[-1,] %>% group_by(pH, NaCl,con_level) %>% summarise(post_pred_pval = mean(prednew > observed))
dRI_df_post$NaCl <- as.factor(dRI_df_post$NaCl)
# 
dRI_df_post_xtable <- dRI_df_post[1:8,4]
for(i in 2:14){
  dRI_df_post_xtable <- cbind(dRI_df_post_xtable, dRI_df_post[((i-1)*8+1):(8*i),4])
}
# 
R_df_post <- R_df[-1,] %>% group_by(pH, NaCl,con_level) %>% summarise(post_pred_pval = mean(prednew > observed))
R_df_post$NaCl <- as.factor(R_df_post$NaCl)
R_df_post_xtable <- R_df_post[1:14,4]
for(i in 2:14){
  R_df_post_xtable <- cbind(R_df_post_xtable, R_df_post[((i-1)*14+1):(14*i),4])
}
#########################
xtable(R_df_post_xtable, digits = 3)
xtable(dRI_df_post_xtable, digits = 3)
########################
# figure of posterior predictive p-values
dRI_df_post %>% ggplot(aes(x=con_level,y=post_pred_pval, color=NaCl)) + geom_line(linetype = "dashed") + geom_point() + facet_wrap(~pH) + geom_hline(yintercept = 0.50, color="red", size=1.5) + xlab("Concentration level") + ylab("Posterior predictive p-values")+ ggtitle("") + theme_bw() + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),strip.background = element_blank(),panel.border = element_rect(colour = "black")) + theme(text = element_text(size=12),
...
R_df_post %>% ggplot(aes(x=con_level,y=post_pred_pval, color=NaCl)) + geom_line(linetype = "dashed") + geom_point() + facet_wrap(~pH) + geom_hline(yintercept = 0.50, color="red", size=1.5) + xlab("Concentration level") + ylab("Posterior predictive p-values")+ ggtitle("") + theme_bw() + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),strip.background = element_blank(),panel.border = element_rect(colour = "black")) + theme(text = element_text(size=12),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       axis.text.x = element_text(angle = 0, hjust=1),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                       axis.text=element_text(size=8))
...
# # prior vs posterior
# #########################
# out <- readRDS("./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_shorter_post_check.rds")
# out_mcmc <- as.mcmc(out)
# # A2
