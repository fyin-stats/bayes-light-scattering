...
#setwd("/home/fyin/light_scattering_new/lysozyme/")
  ...
  for(i in 1:n){
    for(j in 1:nang){
      for(l in 1:L)
      {
        ...
        mu_R[i,j,l] <- f*con[i,l]*pow(n0[l]*dndc,2)*(N[l]+1)*monomer.mass*(1-2*A2[l]*con[i,l]*(N[l]+1)*monomer.mass)
      }
    } 
  }
  # derived quantities
...
#
write.model(lysozyme_model, "./lysozyme/lysozyme_model_M12_log_normal.bug")
############################
time1 <- Sys.time()
...
out <- do.call(jags.parallel, 
               list(data=dat_jags, 
                    parameters.to.save = c("A2","dndc","s2cd","s2R", "s2c", "s2n","con","N"),
                    n.chains = nchains,
                    n.iter = burn+nadapt+draws,
                    n.burnin = burn+nadapt,
                    n.thin = thin, 
                    digits = 16, 
                    model.file="./lysozyme/lysozyme_model_M12_log_normal.bug"))
...
time2 <- Sys.time()
####
saveRDS(out, "./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_M12.rds")
...
# # ########################
# pdf("lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_shorter_residuals_LS.pdf")
# p_R <- ggplot(R_df, aes(x=factor(con_level), y=(prednew-observed)/observed)) + geom_boxplot(outlier.size = 0) + facet_wrap(~pH+NaCl) + geom_hline(yintercept=0, color = "red", size=1)
# print(p_R)
# dev.off()
# # ##########################
# pdf("lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_shorter_residuals_dRI.pdf")
# p_dRI <- ggplot(dRI_df, aes(x=factor(con_level), y=(prednew-observed)/observed)) + geom_boxplot(outlier.size = 0) + facet_wrap(~pH+NaCl) + geom_hline(yintercept=0, color = "red", size=1)
# print(p_dRI)
# dev.off()
# #########################
# pdf("lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_shorter_residuals_con.pdf")
# p_con <- ggplot(con_df[-1,], aes(x=factor(con_level), y=(con/con.meas))) + geom_boxplot(outlier.size = 0) + facet_wrap(~pH+NaCl) + geom_hline(yintercept=1, color = "red", size=1)
# print(p_con)
# dev.off()
# ########################
# #########################
