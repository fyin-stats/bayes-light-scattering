...
lysozyme_model <- function(){
  ...
  for(l in 1:L){
    ...
    # 
    # N[l] ~ dbern(0.8)
    Mass[l] ~ dunif(monomer.mass, monomer.mass*2)
  }
  ...
  for(i in 1:n){
    for(j in 1:nang){
      for(l in 1:L)
      {
        ...
        mu_R[i,j,l] <- f*con[i,l]*pow(n0[l]*dndc,2)*Mass[l]*(1-2*A2[l]*con[i,l]*Mass[l])
      }
    } 
  }
  # derived quantities
...
#
write.model(lysozyme_model, "./lysozyme/lysozyme_model_Mass_log_normal.bug")
############################
time1 <- Sys.time()
...
out <- do.call(jags.parallel, 
               list(data=dat_jags, 
                    parameters.to.save = c("A2","dndc","s2cd","s2R", "s2c", "s2n","con", 
                                           ...
                                           "scat.meas.new", "ri.meas.new","Mass"),
                    n.chains = nchains,
                    n.iter = burn+nadapt+draws,
                    n.burnin = burn+nadapt,
                    n.thin = thin, 
                    digits = 16, 
                    model.file="./lysozyme/lysozyme_model_Mass_log_normal.bug"))
...
time2 <- Sys.time()
####
saveRDS(out, "./lysozyme/lysozyme_complete_pool_constant_dndc_noconmeas_log_normal_loose_prior_Mass.rds")
