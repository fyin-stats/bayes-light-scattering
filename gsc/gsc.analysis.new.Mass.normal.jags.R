...
packages <- c("foreach", "doParallel", 
              "boot", "parallel","rjags", "dplyr", "coda", "ggplot2","R2jags", "R2WinBUGS")
ipak(packages)
...
gsc_model <- function(){
  ...
  mu_Mass ~ dunif(monomer.mass, 20*monomer.mass)
  inv.s2Mass ~ dgamma(1, pow(monomer.mass/3, 2))
  s2Mass <- 1/inv.s2Mass
  Mass ~ dnorm(mu_Mass, inv.s2Mass);T(monomer.mass, 20*monomer.mass)
...
out <- do.call(jags.parallel, 
               list(data=gsc_dat_jags, inits=jags.inits, 
                    ...
                    n.thin = thin, 
                    digits = 16, 
                    model.file="./gsc/gsc_model_log_normal_Mass_normal.bug"))
########
time2 <- Sys.time()
####
